...
public class CharacterChange {
            ...
            if (json.has(currentCharacter)) {
                ...
                if (currentCharacterTransitions.has(triggerType)) {
                    ...
                    if (events.has(keyword)) {
                        ...
                        //ServiceLocator.getIOManager().sendOutput("Warning: The effects for the '" + triggerType + "' event and the '" + currentCharacter + "' character was not found");
                    }
                } else {
                    //ServiceLocator.getIOManager().sendOutput("Warning: The event '" + triggerType + "' for the '" + currentCharacter + "' character was not found");
                }
            } else {
                //ServiceLocator.getIOManager().sendOutput("Warning: The character '" + currentCharacter + "' was not found");
            }
    ...
    public void checkForCharacterChange(Player player) {
        ...
        if (!highestCharacter.equals(currentCharacter)) {
            ...
            ServiceLocator.getIOHandler().sendOutput("You're character type is now changed! You are now a " + highestCharacter + "!");
        }
